...
void StorageSource::add_source(model_delight::NlohmannJsonRequestPtr&& request,
    ...
    // 检查请求字段是否完整
    const auto field_integrity = request_body.contains(schema::key::name) and
                                 request_body.contains(schema::key::url) and
                                 request_body.contains(schema::key::access_key) and
                                 request_body.contains(schema::key::secret_key) and
    ...
    data_source.create_time = util_delight::Date::get_current_timestamp_32();
        ...
        // 检查插入情况
        if (const auto [result, error] =
                service_delight::StorageService::get_instance().append_storage(&data_source);
            ...
            auto response = model_delight::BasicResponse{.code = k500InternalServerError,
                                ...
                                .to_json();
            callback(
                model_delight::NlohmannResponse::new_nlohmann_json_response(std::move(response)));
            ...
            operation_log.user_id =
                bsoncxx::oid{request_body[schema::key::user_id].get<std::string_view>()};
    ...
    auto response =
        model_delight::BasicResponse{.code = k200OK, .message = "Success", .result = "Success"};
    callback(
        model_delight::NlohmannResponse::new_nlohmann_json_response(std::move(response.to_json())));
...
void StorageSource::remove_source(model_delight::NlohmannJsonRequestPtr&& request,
    ...
    const auto field_integrity =
        request_body.contains(schema::key::user_id) and request_body.contains(schema::key::bson_id);
    ...
    const auto bson_id = bsoncxx::oid{request_body[schema::key::bson_id].get<std::string_view>()};

    try {
        const auto [fst, snd] =
            service_delight::StorageService::get_instance().get_storage_by_id(bson_id);

        const auto source_name =
            fst.value().find(schema::key::name)->get_oid().value.to_string();

        if (const auto [result, error] =
            service_delight::StorageService::get_instance().remove_storage(bson_id);
            not result.has_value() or result.value() == false){
                service_delight::Logger::get_instance().log(
                service_delight::ConsoleLogger | service_delight::BasicLogger,
                spdlog::level::info,
        ...
        auto response = model_delight::BasicResponse{
            .code = k400BadRequest,
            .message = "StorageSource::remove_source failed",
            .result = e.what()
        };
        callback(model_delight::NlohmannResponse::new_nlohmann_json_response(std::move(response.to_json())));
...
void StorageSource::list_all_source(model_delight::NlohmannJsonRequestPtr&& request,
    ...
    if (not result.has_value()) {
        ...
        auto response = model_delight::BasicResponse{
            .code = k400BadRequest,
            .message = "StorageSource::list_all_source failed",
            .result = error.data()
        };
       callback(model_delight::NlohmannResponse::new_nlohmann_json_response(std::move(response.to_json())));
        return;
...
void StorageSource::list_all_ids(model_delight::NlohmannJsonRequestPtr&& request,
    ...
    if (not result.has_value()) {
        ...
        auto response = model_delight::BasicResponse{
            .code = k400BadRequest,
            .message = "StorageSource::list_all_ids failed",
            .result = error.data()
        };
        callback(model_delight::NlohmannResponse::new_nlohmann_json_response(std::move(response.to_json())));
        return;
...
void StorageSource::get_one_source(model_delight::NlohmannJsonRequestPtr &&request, std::function<void(const HttpResponsePtr &)> &&callback){
    service_delight::Logger::get_instance().log(
        service_delight::ConsoleLogger | service_delight::BasicLogger,
        spdlog::level::info,
    ...
    const auto field_integrity =
        request_body.contains(schema::key::user_id)
    and
        request_body.contains(schema::key::bson_id);
    ...
    const auto user_id = bsoncxx::oid{request_body[schema::key::user_id].get<std::string_view>()};
    const auto data_source_id = bsoncxx::oid{request_body[schema::key::bson_id].get<std::string_view>()};
    ...
    if (not result.has_value()) {
        ...
        auto response = model_delight::BasicResponse{
            .code = k400BadRequest,
            .message = "StorageSource::get_one_source failed",
            .result = error.data()
        };
        callback(model_delight::NlohmannResponse::new_nlohmann_json_response(std::move(response.to_json())));
        return;
