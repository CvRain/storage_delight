...
namespace service_delight {
    ...
    public:
        void init();
        schema::result<schema::DbGroup, std::string> add_group(schema::DbGroup *group);
        schema::result<schema::DbGroup, std::string> get_group(const std::string &group_id);
        schema::result<bool, std::string> group_exist(const std::string &group_id);
    private:
